...
|------------|------------------|------------|
| Ingesta | Construir snapshot de 15 min (clima + incidentes + GPS), usando Open-Meteo si está disponible o respaldo DGT si no lo está, y fusionar DATEX2 DGT | Python / NiFi (opcional) |
...
| Visualización | Operación, rutas/métricas, **guía del ciclo KDD** (fases, payload, topología vs mapa), **cuadro de mando** (consultas supervisadas, **modelo de datos** por plantilla, **análisis Hive** heurístico, informes PDF, flota multi-camión, simulación en mapa, correo SMTP opcional) + asistente de flota | Streamlit + Folium + Altair (topología en pestaña KDD) + PyHive/pandas |
...
2. Si Open-Meteo (u OpenWeather si está configurado) entrega una respuesta válida, `clima_hubs` se publica con `source=openmeteo` o `source=openweather`; si no, la ingesta reutiliza el contexto meteorológico inferido desde DGT y marca `fallback_activo=true`.
...
8. La pestaña **Ciclo KDD** del dashboard enlaza fases con scripts y ficheros del repo, permite probar **clima en vivo** (Open-Meteo por defecto; OpenWeather opcional con clave) y muestra **una** vista topológica de la red para fases 3–5 (detalle en `docs/DASHBOARD_KDD_UI.md`).
...
- **Clima degradado controlado**: la API de clima primaria (Open-Meteo por defecto) puede fallar; cuando la respuesta no es usable, el sistema deriva clima operativo desde DATEX2 y conserva el mismo contrato JSON.
...
- `clima_hubs` / `clima`: incorpora `source` (`openmeteo`, `openweather` o `dgt`) y `fallback_activo` para distinguir dato principal frente a dato alternativo.
...
- NiFi conserva atributos `simlog.provenance.*` para inspeccionar en `Data Provenance` qué parte del snapshot procede de simulación, Open-Meteo (u OpenWeather legacy) y DGT.
- El modo actual documentado prioriza la continuidad del pipeline con información alternativa DGT cuando la API de clima primaria no aporta datos válidos.

## Configuración del respaldo climático (DGT)

- `ingesta/ingesta_kdd.py` combina `consulta_clima_hubs()` con `obtener_incidencias_dgt()` mediante `combinar_clima_hubs(...)`.
- Si Open-Meteo falla (red, timeout) o OpenWeather devuelve `401` / JSON inválido cuando está activo, la función usa `info_dgt["clima_hubs"]` y expone `fallback_activo=true`.
- En CLI y Airflow, el comportamiento es automático; no requiere intervención adicional si `SIMLOG_USE_DGT` está habilitado.
- En NiFi, `OpenMeteo_InvokeHTTP` intenta enriquecer el payload y `MergeOpenWeatherIntoPayload.groovy` solo incorpora clima cuando la respuesta es válida. Después `MergeDgtDatex2IntoPayload.groovy` reconstruye `clima_hubs` desde incidencias DATEX2 si `simlog.weather.available=false` o el clima está vacío.
- OpenWeather solo requiere clave: `.env` con `SIMLOG_WEATHER_PROVIDER=openweather` y `OWM_API_KEY` / `API_WEATHER_KEY`. NiFi usa Open-Meteo multi-hub sin clave por defecto.
